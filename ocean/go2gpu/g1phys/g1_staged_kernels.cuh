...
__global__ void k5_assemble(int n, const float* __restrict__ g_qpos,
    ...
    if (lane == 0) {
        ...
        cnt[0] = nc;
        // Go2 foot-contact flags + collision count for gait-shaping rewards
        // (active = dist < 0). Foot sphere geoms: FL=20 FR=32 RL=44 RR=56; any
        // OTHER collision sphere touching the floor counts as a body collision.
        // Layout per env (stride 5): [FL,FR,RL,RR, n_body_collisions].
        {
            float fl = 0.0f, fr = 0.0f, rl = 0.0f, rr = 0.0f, coll = 0.0f;
            for (int c = 0; c < nc; c++) {
                if (cdist[c] < 0.0f) {
                    int gg = g1c_pair_geom2[cpair[c]];
                    if (gg == 20) fl = 1.0f;
                    else if (gg == 32) fr = 1.0f;
                    else if (gg == 44) rl = 1.0f;
                    else if (gg == 56) rr = 1.0f;
                    else coll += 1.0f;
                }
            }
            g_footc[5 * e + 0] = fl;
